// META_JAVASCRIPT Version Alpha 1.2b
...
// Se corrige error que mustra texto agregar y eliminar linea al poner el cursor en los iconos actulizar y QR
...
// Se corrige mostar borde elipses
// Se corrige mostrar figuras con fondo negro en configuracion y fondo blanco en raton (ahora en todas las pestañas las muestra fondo blanco por defecto)
...
import com.cage.zxing4p3.*; // librería codigo QR

import http.*; // libreria servidor local
import java.util.Map; // librería para extraer los argumentos de la URL para darle comandos al servidor local

import java.awt.event.KeyEvent; // librería para leer teclas espciales

SimpleHTTPServer server;
String data="Ready"; // variable para devolver respuesta al llamado del servidor local


ZXING4P zxing4p; //codigo QR
...
boolean generated = false; // bandera para saber si ya se generó el código QR

String  ip4QR; // la URL para codificar como código QR
...
Grayscale  =   RGB
  ...
  void setup() {
    ...
    // se inicializa variable para genera códigos QR
      zxing4p = new ZXING4P();
    
    // se inicializa la variable del servidor local y se desactiva la plantilla .html y se activa la pantilla .ftl
    SimpleHTTPServer.useIndexHtml = false;
    server = new SimpleHTTPServer(this); 
    
    // consulta dirección ip actual
  ...
  void draw() {
    ...
    */
    ...
    // reslata boton Actualizar
    if (mouseX>75 && mouseX<100 && mouseY>10 && mouseY<34) {
      fill(128);
      rect(75, 10, 26, 25); // boton variable
      fill(255); 
      text(idiomagui.getString("Actualizar"),15, height-22);
    }
    
    
    // reslata boton QR
    if (mouseX>125 && mouseX<175 && mouseY>10 && mouseY<34) {
      fill(128);
      rect(125, 10, 26, 25); // boton variable
      fill(255); 
      text(idiomagui.getString("Generar")+" QR / "+idiomagui.getString("Proyecto")+": "+proyectonombre,15, height-22);
    }
...
/////////////////////
    ...
    if (y>0 && y <=tlineas && mouseY<height-71 && tlineas >= icodigo+y ) {
      ...
      */
      ...
      if (mouseX>widthline+50 && mouseX<widthline+70 && mouseY > 40) {
        noStroke();
      ...
      if (mouseX>widthline+80 && mouseX<widthline+100 && mouseY > 40) {
...
//Ftext(icodigo+"  "+(linearaton-1)+"   "+lineaactual,mouseX, mouseY);
  ...
  void keyPressed() {
    ...
    if(keyCode==KeyEvent.VK_F1){
      ...
      servidorlocal(); // actualiza servidor local
      generaQR();
  ...
  void mousePressed() {
    ...
    if (mouseX>20 && mouseX<40 && mouseY>10 && mouseY<30) {
      ...
      //link( "http://192.168.0.18:8000/"+proyectonombre+".html");
      link( "http://"+ip+":8000/"+proyectonombre+".html");
      ...
      if( OS.contains("windows") == true) {
        ...
        URL = "http://"+ip+":8000/"+proyectonombre+".html";
        URL = URL.replace('\\', '/');
        link(URL);
      }  
...
void guardameta(){
  ...
  void guardajs(int modo){
      ...
      String[] texto, textof, lib, setup, setupf, teclapresionada, ratonpresionado;
...
*/
      ...
      texto[0]="";
      for (int i=1; i-1<tlineasconfig; i++) {
        if (cargaconfiguracion.getJSONObject(i-1).isNull("js") == false) {
          texto = expand(texto, i);
          texto [i-1] ="";
          ...
          texto [i-1] = texto [i-1]+cargaconfiguracion.getJSONObject(i-1).getString("js"); 
...
//println(texto[1]);
      ...
      if (libvideo==true) textof [0] = textof [0] +"    <video id=\"video\"> </video>";
   ...
   textof = expand(textof, textof.length+1);
      ...
      textof = expand(textof, textof.length+1);
      textof [textof.length-1]= "function velicidad(cuadros) {\n    let fechaactual = null;\n    const fecha = Date.now();\n    do {\n        fechaactual = Date.now();\n    } while (fechaactual - fecha < cuadros);\n}\n";
    ...
    //println(textof[2]);
      ...
      textof = expand(textof, textof.length+1);
      //textof [textof.length-1]= "\nvelicidad(600-("+velocidad+"*10));\n}\n\n setup();\n anim();"; // cierra la función anim()
      textof [textof.length-1]= "\nvelicidad(600-("+velocidad+"*10));\n}"; // cierra la función anim()
      //textof [textof.length-1]= "\n}"; // cierra la función anim()
  ...
  void guardapde(int modo){
      ...
      //lib[0]=lib[0]+"// Generated with Meta_Javascript Alpha 1.2b\n// https://github.com/hiteclab/Meta_Javascript/releases\n";
...
void leelinea(int i) {
  ...
  // muestra hipervínculo
  if (id==70) {
    line=""+idiomaactual.get(str(id))+" ( "+objeto.get("hipervinculo")+" )";
  } else   
 
    // muestra leerargumento
  if (id==71) {
    line=""+idiomaactual.get(str(id))+" ( "+objeto.get("variable")+" , "+objeto.get("valor")+" )";
  } else   
...
void servidorlocal(){
      // activa el servidor local con los archivos generados
      TemplateFileHandler templateHandler = new ResultFromPost("index.ftl");
      server.createContext("", templateHandler);
      server.serveAll("",sketchfolder+proyectonombre);
}

void generaQR(){      
      qr=true;

      ip4QR = "http://"+ip+":8000/"+proyectonombre+".html"; // se crea la cadena competa con la ip actual, que se usará para generar el código QR
      try {
        QRCode = zxing4p.generateQRCode(ip4QR, 400, 400);
...
class ResultFromPost extends TemplateFileHandler {
  ...
  void createMap() {
    ...
    // Arduino salida digital HIGH
    if (params.containsKey("asdh")) {
      try { 
    arduino.digitalWrite(int(params.get("asdh")), Arduino.HIGH); data= "1"; } catch (Exception e) {}  // prende el pin
      //  Ejemplo:
      //  http://localhost:8000/?asdh=13
    }
    
    // Arduino salida digital LOW
    if (params.containsKey("asdl")) {
      try { 
    arduino.digitalWrite(int(params.get("asdl")), Arduino.LOW); data= "0"; } catch (Exception e) {}// apaga el pin
      //  Ejemplo:
      //  http://localhost:8000/?asdl=13
    } 
    
    // Arduino entrada digital
    if (params.containsKey("aed")) {
      try {  arduino.pinMode(int(params.get("aed")), arduino.INPUT_PULLUP);
             data = str(1-arduino.digitalRead(int(params.get("aed"))) ); 
    } catch (Exception e) {}  // lectura digital del pin
      //  Ejemplo:
      //  http://localhost:8000/?aed=12
    }

    // Arduino entrada analogica
    if (params.containsKey("aea")) {
      try {  data = str(arduino.analogRead(int(params.get("aea"))) ); } catch (Exception e) {}  // lectura digital del pin
      //  Ejemplo:
      //  http://localhost:8000/?aea=2
    }
    ...
    if (params.containsKey("am")) {
      try { 
          data = "aservo";
          arduino.pinMode(int(params.get("am")), Arduino.SERVO); // pone el pin en modo servo
          arduino.servoWrite(int(params.get("am")), int(params.get("pos")) ); // mueve el servo a la posición indicada
    
      } catch (Exception e) {}  // prende el pin
      //  Ejemplo:
      //  http://localhost:8000/?am=7&pos=90
    }
    ...
    // IotControllerAP salida digital HIGH
    if (params.containsKey("isdh")) {
      
      data= "1";
      loadStrings("http://192.168.4.1/?SDPIN="+params.get("isdh")+"&STATE=1");

      //  Ejemplo:
      //  http://localhost:8000/?isdh=13
    }
    
    // IotControllerAP salida digital LOW
    if (params.containsKey("isdl")) {
      
      data= "0";
      loadStrings("http://192.168.4.1/?SDPIN="+params.get("isdl")+"&STATE=0");

      //  Ejemplo:
      //  http://localhost:8000/?isdl=13
    } 
    
    // IotControllerAP entrada digital
    if (params.containsKey("ied")) {
      
      data = loadStrings("http://192.168.4.1/?EDPIN="+params.get("ied"))[0]; // entrada digital
      
      //  Ejemplo:
      //  http://localhost:8000/?ied=0
    }

    // IotControllerAP entrada analogica
    if (params.containsKey("iea")) {
      
      data = loadStrings("http://192.168.4.1/?EAPIN="+params.get("iea"))[0]; // entrada analógica
            
      //  Ejemplo:
      //  http://localhost:8000/?iea=0
    }

    // IotControllerAP Servo Motor
    if (params.containsKey("im")) {
      data= "iservo";
      loadStrings("http://192.168.4.1/?SMPIN="+params.get("im")+"&ANGLE="+params.get("pos")); // servo motor
      
      //  Ejemplo:
      //  http://localhost:8000/?im=0&pos=90
    }
    
    // actualiza el dato a mostrar en la pagina web que se abre al llamar al servidor local
