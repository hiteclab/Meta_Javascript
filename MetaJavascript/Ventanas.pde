...
class PWindow3 extends PApplet {
  ...
  void settings() {
    ...
    if (idlee==70) temp = objetoactual.getString("hipervinculo");
  ...
  void draw() {
    ...
    if (lastid == 71) aleatorio(); // dividir
  ...
  void mousePressed() {
    ...
    if (lastid==-1) {
      ...
      if (mouseX>width/3) {
        ...
        if (posy==220) { // opcion hipervinculo
          lastid=70;
          
        } 
        if (posy==240) { // opcion argumentos
          lastid=71;
          
        } 
        if (posy==280) { // opcion código nativo
          lastid=40;
          instru="";
        } 
        /*
    ...
    if (lastid == 10 || lastid == 11 || lastid == 12 || lastid == 13 || lastid == -10 || lastid == -11 || lastid == -12 || lastid == -13) {  // ARDUINO
      ...
      if (posy== 220 && mouseX>170 && mouseX<260) {
        ...
        // entradigital Arduino                       
        if (lastid == 11) linea = "fetch('http://"+ip+":8000/?aed="+pin+"')\n.then(response => response.text())\n.then(data => "+px+" = data);";
        // entradaanalogica Arduino
        if (lastid == 12) linea = "fetch('http://"+ip+":8000/?aea="+pin+"')\n.then(response => response.text())\n.then(data => "+px+" = data);";
        // servo Arduino
        if (lastid == 13) linea = "fetch(\""+ip+":8000/?am="+pin+"&pos="+px+"\");";
   
        // entradigital IoTcontrollerAP
        if (lastid == -11) linea = "fetch('http://"+ip+":8000/?ied="+pin+"')\n.then(response => response.text())\n.then(data => "+px+" = data);";
        //entradaanalogica IoTcontrollerAP
        if (lastid == -12) linea = "fetch('http://"+ip+":8000/?iea="+pin+"')\n.then(response => response.text())\n.then(data => "+px+" = data);";
        // servo IoTcontrollerAP
        if (lastid == -13) linea = "fetch(\""+ip+":8000/?im="+pin+"&pos="+px+"\");";
    ...
    if (lastid == 97 || lastid == 61 || lastid == 62 || lastid == 63 || lastid == 64 || lastid == 65 || lastid == 71) {  // ALEATORIO
      ...
      if (posy== 220 && mouseX>170 && mouseX<260) {
        ...
        if (lastid == 71) renuevaobjeto.setString("instruccion", "leerargumento");
        ...
        if (lastid == 71) linea = "let searchParams = new URLSearchParams(location.search);\n "+px+" = searchParams.get('"+py+"');";

        renuevaobjeto.setInt("id", lastid);
        renuevaobjeto.setString("variable", px);
        if (lastid == 97) renuevaobjeto.setInt("vmaximo", int(py));
        if (lastid == 61 || lastid == 62 || lastid == 63 || lastid == 64 || lastid == 65 || lastid == 71) renuevaobjeto.setString("valor", py);
    ...
    if (lastid == 109 || lastid == 118) {  // IMAGEN || VIDEO
      ...
      if (posy== 220 && mouseX>170 && mouseX<260) {
        ...
        if (lastid == 118)linea = "video.setAttribute(\"src\", \"data/"+temp+"\");\n video.currentTime = 0;\n video.load();\n video.play();\n video.style.left = "+px+"+'px';\n video.style.top = "+py+"+'px';";
    ...
    // botones cambiar opciones en configuración
    if (config==true) {
  ...
  void keyPressed() {
    ...
    if (lastid== 70) temp = typetext(temp); // hipervínculo
    if (lastid== 71) py = typetext(py); // leerargumento
...
void qrwindow(){
    ...
    if (generated) {
    set(0, 0, QRCode); // DISPLAY GENERATED IMAGE
    fill(0);
    text(ip4QR, 10, height-10);
  }
  ...
  void idioma() {
    ...
    y= ((posy-40)/20)+1;
    fill(120);
    if (posy>20 && posy< 360)rect(0, posy, width, 20);
  ...
  void splash() {
    surface.setTitle("META_JAVASCRIPT Version Alpha 1.2b");
    ...
    fill(255);
    text("META_JAVASCRIPT Version Alpha 1.2b", 255, 340);
    fill(120);
  ...
  void instruccion() {
    ...
    if (posy>0 && posy!=100 && posy!=200 && posy!=260 && posy< 300 && posy!=620 && mouseX>width/3)rect(width/3, posy, width/3, 20);
    ...
    fill(120);
    text("HTML", (width/3)+10, 217);
    fill(255);
    text(""+idiomaactual.get(str(70)), (width/3)+40, 237);
    text(""+idiomaactual.get(str(71)), (width/3)+40, 257);
    
    fill(120);
    text(idiomagui.getString("Avanzadas"), (width/3)+10, 277);
    fill(255);
    text(idiomagui.getString("código")+" "+idiomagui.getString("nativo"), (width/3)+40, 297);
  ...
  void aleatorio() {
            ...
            if (lastid== 63 && objeto.isNull("valor") == false) py= ""+objeto.get("valor"); 
              ...
              if (lastid== 64 && objeto.isNull("valor") == false) py= ""+objeto.get("valor"); 
                ...
                if (lastid== 65 && objeto.isNull("valor") == false) py= ""+objeto.get("valor"); 
                   ...
                   if (lastid== 71 && objeto.isNull("valor") == false) py= ""+objeto.get("valor"); 
                   else py="";
      aleatorio=true;
    }
    ...
    if (lastid == 71)text(idiomagui.getString("Parámetro")+":", 30, 95);
    ...
    if (mousePressed== true) {
      ...
      if (posy== 100 && mouseX>170 && mouseX<260) {
        ...
        //if (falsetrue==false){if(lastid==10)linea = "arduino.digitalWrite( "+pin+", Arduino.LOW );"; if(lastid==-10) linea = "loadStrings(\"http://"+configuration.getJSONObject(3).getString("ip")+"/?SDPIN="+pin+"&STATE=0\");"; }
                           //if (lastid == 11) linea =  "var read = new XMLHttpRequest();\n read.open(\"GET\", \"http://"+ip+":8000/?aed="+pin+"\", false);\n read.overrideMimeType(\"text/html\");\n read.onreadystatechange = function() {\n    if (read.readyState === 4) if(read.status === 200 || read.status == 0) {\n      "+px+" = read.responseText;\n}\n}\nread.send();";
        
        //if (falsetrue==false){if(lastid==10) linea = "var read = new XMLHttpRequest();\n read.open(\"GET\", \"http://"+ip+":8000/?asdl="+pin+"\", true);\n read.overrideMimeType(\"text/html\");\n read.send();";}
        //if (falsetrue==false){if(lastid==10) linea = "var read = new XMLHttpRequest();\n read.open(\"POST\", \"http://"+ip+":8000/?asdl="+pin+"\", false);\n read.overrideMimeType(\"text/html\");\n read.send();";}
        
        
        //if (falsetrue==false){if(lastid==10) linea =  "var read = new XMLHttpRequest();\n read.open(\"GET\", \"http://"+ip+":8000/?asdl="+pin+"\", false);\n read.overrideMimeType(\"text/html\");\n read.onreadystatechange = function() {\n    if (read.readyState === 4) if(read.status === 200 || read.status == 0) {\n      var t = read.responseText;\n}\n}\nread.send();";}
        if (falsetrue==false){if(lastid==10) linea = "fetch(\""+ip+":8000/?asdl="+pin+"\");"; if(lastid==-10) linea = "fetch(\""+ip+":8000/?isdl="+pin+"\");";}
        //if (falsetrue==false){if(lastid==10) linea = "var read = new XMLHttpRequest();\n read.open(\"GET\", \"http://"+ip+":8000/?asdl="+pin+"\", true);\n read.send();";}
      ...
      //else {if(lastid==10)linea = "arduino.digitalWrite( "+pin+", Arduino.HIGH );"; if(lastid==-10) linea = "loadStrings(\"http://"+configuration.getJSONObject(3).getString("ip")+"/?SDPIN="+pin+"&STATE=1\");"; }
        //else {if(lastid==10) linea = "var read = new XMLHttpRequest();\n read.open(\"GET\", \"http://"+ip+":8000/?asdh="+pin+"\", true);\n read.overrideMimeType(\"text/html\");\n read.send();";}
        //else {if(lastid==10) linea = "var read = new XMLHttpRequest();\n read.open(\"POST\", \"http://"+ip+":8000/?asdh="+pin+"\", false);\n read.overrideMimeType(\"text/html\");\n read.send();";}
        
        //else {if(lastid==10) linea =  "var read = new XMLHttpRequest();\n read.open(\"GET\", \"http://"+ip+":8000/?asdh="+pin+"\", false);\n read.overrideMimeType(\"text/html\");\n read.onreadystatechange = function() {\n    if (read.readyState === 4) if(read.status === 200 || read.status == 0) {\n      var t = read.responseText;\n}\n}\nread.send();";}
        else {if(lastid==10) linea = "fetch(\""+ip+":8000/?asdh="+pin+"\");"; if(lastid==-10) linea = "fetch(\""+ip+":8000/?isdh="+pin+"\");";}
  ...
  void config() {
    ...
    //text(idiomagui.getString("Configuración")+": Meta_Javascript "+ver[0], 10, 20); //else text(idiomagui.getString("Variable"), 10, 20);
    text(idiomagui.getString("Configuración")+":", 10, 20); //else text(idiomagui.getString("Variable"), 10, 20);

    text("Meta_Javascript "+ver[0], 50, 57);
    ...
    */
    ...
    //text(idiomagui.getString("Nuevo")+" Meta_Javascript: "+newver[0], 50, 127);
    text(idiomagui.getString("Ultima version")+" Meta_Javascript: "+newver[0], 50, 127);
    noStroke();
    fill(20);
    rect(41, 140, 90, 20);
    if (posy== 140 && mouseX>41 && mouseX<131) {
  ...
  void coment() {
    ...
    if (mousePressed== true) {
      ...
      if (posy== 80 && mouseX>170 && mouseX<260) {
        JSONObject renuevaobjeto = new JSONObject();
        String linea ="";
        if(lastid == 0) linea = "//"+instru;
        if(lastid == 70) linea = "window.location.replace(\"http://"+temp+"\");";
        if (objeto.isNull("para") == false)renuevaobjeto.setBoolean("para", true);
        if (objeto.isNull("si") == false)renuevaobjeto.setBoolean("si", true);
        if (objeto.isNull("sino") == false)renuevaobjeto.setBoolean("sino", true);
        if (objeto.isNull("encapsulado") == false)renuevaobjeto.setBoolean("encapsulado", true);
        if (lastid==70)renuevaobjeto.setString("instruccion", "hipervinculo");
        if(lastid == 0) renuevaobjeto.setString("comentario", instru);
        if(lastid == 70) renuevaobjeto.setString("hipervinculo", temp);
        renuevaobjeto.setInt("id", lastid);
        renuevaobjeto.setString("js", linea);
        println(renuevaobjeto);
        if (codetab==0) cargacodigo.setJSONObject(lineaclik, renuevaobjeto);         
    ...
    background(200);
    fill(80);
    if(lastid == 0) text(idiomagui.getString("Comentario"), 10, 20); //else text(idiomagui.getString("Variable"), 10, 20);
    if(lastid == 70) text(idiomaactual.getString("70"), 10, 20); //else text(idiomagui.getString("Variable"), 10, 20);

    rect(40, 40, width, 20);  // area de texto valor
    fill(255);
    if(lastid == 0)text(instru+"|", 42, 55);
    if(lastid == 70)text(temp+"|", 42, 55);
  ...
  void proyecto() {
    ...
    if (mousePressed== true) {
      ...
      if (posy== 80 && mouseX>170 && mouseX<260) {        
        ...
        global [0] = "// Generated with Meta_Javascript Alpha 1.2b\n// https://github.com/hiteclab/Meta_Javascript/releases\n";
